...
export function RoleListView() {
  ...
  const canCreate = user?.permissions?.includes('roles.create');
  const canUpdate = user?.permissions?.includes('roles.update');
  const canDelete = user?.permissions?.includes('roles.delete');
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        getActions: (params) => [
          ...(canUpdate ? [<CustomGridActionsCellItem showInMenu label="Editar" icon={<Iconify icon="solar:pen-bold" />} href={paths.dashboard.role.edit(params.row.id)} />] : []),
